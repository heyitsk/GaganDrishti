...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Save token if returning one, or just mark as logged in
      if (data.token) {
        localStorage.setItem("gagandrishti_token", data.token);
      }
      localStorage.setItem("gagandrishti_user", username);
    ...
    <div className="flex min-h-screen items-center justify-center bg-[#0B0F19] p-4 font-sans text-slate-200">
      <div className="w-full max-w-md space-y-8">
        <div className="text-center">
          ...
          <p className="text-sm text-slate-400">Sign in to GaganDrishti to manage your cloud security</p>
        </div>

        <form onSubmit={handleLogin} className="rounded-xl border border-slate-800 bg-[#131A2B] p-6 space-y-5 shadow-xl">
        ...
        <p className="text-center text-sm text-slate-400">
          ...
          <Link to="/register" className="font-medium text-[#3B82F6] hover:underline">Register</Link>
        </p>
      </div>
    </div>
  );
